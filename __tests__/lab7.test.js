describe('Basic user flow for Website', () => {
  async function getProductButtonsText() {
    return await page.$$eval('product-item', prodItems => {
      return prodItems.map(item => item.shadowRoot.querySelector('button').innerText);
    });
  }

  async function clickAllProductButtons() {
    await page.$$eval('product-item', prodItems => {
      prodItems.forEach(item => item.shadowRoot.querySelector('button').click());
    });
  }

  async function reloadAndWaitForProducts() {
    await page.reload();
  ...
  beforeAll(async () => {
    ...
    await reloadAndWaitForProducts();
  });
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    prodItemsData.forEach((value, index) => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const buttonText = await getProductButtonsText();
    const cartCount = await page.$eval('#cart-count', element => element.innerText);

    expect(buttonText.every(text => text === 'Remove from Cart')).toBe(true);
    expect(cartCount).toBe('20');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    expect(buttonText.every(text => text === 'Add to Cart')).toBe(true);
    expect(cartCount).toBe('0');
